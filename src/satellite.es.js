...
function sgp4(satrec, tsince) {
    ...
    var r = {
        x: mrt * ux * earthRadius * 1000,
        y: mrt * uy * earthRadius * 1000,
        z: mrt * uz * earthRadius * 1000
    };
...
function eciToEcf(eci, gmst) {
    ...
    // [Y]  =  [-S C  0][Y]
    // [Z]ecf  [0  0  1][Z]eci
    let x = eci.x * Math.cos(gmst) + eci.y * Math.sin(gmst);
    let y = eci.x * -Math.sin(gmst) + eci.y * Math.cos(gmst);
    let z = eci.z;
    return {
        x: x,
        y: y,
